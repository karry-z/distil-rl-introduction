	...
	- Algorithm
		...
		- Loop forever:
			...
			- `(f)` If $S$ is the terminal state, then $S \leftarrow StartingState$ and restart, Else loop repeat $n$ times:
				...
				- $R, S\prime \leftarrow Model(S, A)$
				- $R = R + \kappa \sqrt \tau_{S,A,R,S\prime}$
				- $Q(S, A) \leftarrow Q(S, A) + \alpha [R + \gamma max_a Q(S\prime, a) - Q(S, A)]$
	
	- Notes
		- Modification 1: the reward is now $R + \kappa \sqrt \tau$, where $\kappa$ is a constant and $\tau$ indicates the amount of time steps that the transition $(S,A) \rightarrow (R, S\prime)$ has not been tried during real experience. 
